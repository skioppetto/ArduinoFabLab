const int I_KEYS = A3;
const int O_SR_DATA = 0;
const int O_SR_LATCH = 4;
const int O_SR_CLOCK = 2;
const int O_BUZZ = 1;
...
const unsigned int BLUE_FREQ = 179;
const unsigned int RED_FREQ = 159;
const unsigned int GREEN_FREQ = 142;
const unsigned int YELLOW_FREQ = 127;
const unsigned int DEFEAT_FREQ = 239;
...
const int DISPLAY_RECORD_MS = 2000;
...
byte reproduced[99] = {};
unsigned int reproducedCount = 0;
...
void showScore(int score){
  showScore(score, DISPLAY_NUMBER_MS);
  }

void showScore(int score, int displayDelay) {
  int dec = score / 10;
  int unit = score % 10;
  if (dec > 0) {
    displayNumber(dec);
  ...
  displayNumber(unit);
  delay((dec > 0) ? displayDelay : displayDelay * 2);
  updateSr(SR_OFF);
}
...
void playLedNote(int note) {
  tinyTone(O_BUZZ, NOTES_FREQ[note - 1]);
...
void tinyTone(int pinNumber, unsigned char divisor)
{
